...
for i in $TWEAK_IO $TWEAK_NOATIME $TWEAK_VM_SWAPPINESS $TWEAK_VM_DIRTY $TWEAK_SCHEDULER $TWEAK_SDCARD; do
  if [ "$(getprop $i)" = "" ]; then
    setprop $i enabled;
  fi;
done;

for i in $TWEAK_STAGEFRIGHT $TWEAK_GOVERNOR_CONSERVATIVE $TWEAK_SCHEDULER_DEADLINE $TWEAK_MINFREE $TWEAK_TOUCHSCREEN; do
  if [ "$(getprop $i)" = "" ]; then
    setprop $i disabled;
  fi;
done;
